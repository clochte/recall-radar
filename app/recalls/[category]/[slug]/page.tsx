...
import { getHazardInfo, getSourceAgencyBlock, getHazardWhatToDo, getRelatedArticles } from '@/lib/hazard';
...
export default async function RecallDetailPage({ params }: Props) {
  ...
  const agency = getSourceAgencyBlock(recall);
  const whatToDo = getHazardWhatToDo(recall);
  const relatedArticles = getRelatedArticles(recall);
    ...
    <div className="flex gap-6 items-start">
      <div className="flex-1 min-w-0">
        <div className="max-w-2xl mx-auto py-6">
          ...
          <article>
            ...
            <div className="bg-card border border-border rounded-lg p-5 space-y-4 mb-6">
              ...
              <div className="flex flex-wrap gap-6 text-sm">
                ...
                <div>
                  <p className="text-xs font-semibold text-muted uppercase tracking-wide mb-0.5">Issuing Agency</p>
                  <p className="text-gray-700">{agency.abbreviation}</p>
                </div>
              </div>
            </div>
            ...
            {/* What to do — hazard-specific steps */}
            ...
            {/* Issuing agency context — unique per agency/source */}
            <div className="bg-card border border-border rounded-lg p-5 mb-6 text-sm text-gray-700 leading-relaxed">
              <p className="font-semibold text-navy mb-2">About the {agency.abbreviation}</p>
              <p>{agency.body}</p>
              <a
                href={agency.website}
                target="_blank"
              ...
              >
                View all {agency.abbreviation} recalls →
              </a>
            </div>
                ...
                __html: JSON.stringify([
                  {
                    ...
                    keywords: `recall, ${recall.category}, ${recall.brand ?? ''}, ${agency.abbreviation}, safety alert`,
                  },
                  {
                    '@context': 'https://schema.org',
                    '@type': 'BreadcrumbList',
          ...
          {/* Related articles — contextual, not static */}
          {relatedArticles.length > 0 && (
            <div className="mt-10 pt-6 border-t border-border">
              <p className="text-xs font-semibold text-muted uppercase tracking-wide mb-3">Related reading</p>
              <div className="space-y-3">
                {relatedArticles.map((article) => (
                  ...
                  >
                    ...
                    <div>
                      <span className="text-sm text-navy group-hover:underline font-medium">{article.title}</span>
                      <p className="text-xs text-gray-500 leading-relaxed mt-0.5 line-clamp-2">{article.intro}</p>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          <div className="mt-6 pt-4 border-t border-border flex flex-wrap gap-4 text-sm">
            <Link href={`/${recall.category}`} className="text-navy-light hover:underline">
              ← View all {CATEGORY_LABELS[recall.category]} recalls
            </Link>
            <Link href="/safety-guide" className="text-navy-light hover:underline">
              Consumer safety guide
            </Link>
            <Link href="/faq" className="text-navy-light hover:underline">
              Recall FAQ
            </Link>
          </div>
        </div>
